class DashboardsController < ApplicationController
  ...
  def show
    ...
    # if kenny_loggins == User.find(params[:id])   #<--- no test written to test whether a sessioned user can view someone else's view
      ...
      @wagered_total = (@account.wagers.where(winner_id: nil).where('status != ?', 'declined').where('status != ?', 'expired').sum(:amount) / 100) + (Wager.where(wageree_id: kenny_loggins.id, status: "accepted").where(winner_id: nil).sum(:amount) / 100)
